...
if st.sidebar.button("Charger Données"):
    ...
    # Mesures de performance (sur backtest simple)
    @st.cache_data
    def backtest(weights, returns):
        ...
        sortino = (port_returns.mean() * 252 - 0.02) / downside
    ...
    fig_ef.add_trace(go.Scatter(x=[port_vol], y=[
                     port_ret], mode='markers', name='Portefeuille Optimal', marker=dict(size=10, color='red')))
